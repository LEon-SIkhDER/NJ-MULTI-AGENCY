import { useRef, useState, useId, type SyntheticEvent } from "react";
...
type props = { children: React.ReactNode; className: string, refetch: UseQueryResult["refetch"], user: User, approveStatus: string }
const ApproveAsPitcher = ({ children, className, refetch, user, approveStatus }: props) => {
    const rawId = useId();
    const uniqueId = (user?._id || user?.uid || rawId).replace(/[^a-zA-Z0-9_-]/g, "");
    const photoInputId = `pitcher-photo-${uniqueId}`;
    const nidInputId = `nid-images-${uniqueId}`;
                ...
                <dialog ref={modalRef} className="modal">
                    ...
                    <div className="modal-box relative w-11/12 max-w-2xl p-0 overflow-hidden"
                        ...
                        <div className="flex items-center justify-between px-6 pt-6 pb-5" style={{ borderBottom: "1px solid hsl(222 10% 14%)" }}>
                            <div>
                                <h3 className="font-display text-xl font-bold tracking-tight" style={{ color: "hsl(220 20% 94%)" }}>Approve as Pitcher</h3>
                                <p className="text-xs mt-0.5" style={{ color: "hsl(220 10% 52%)" }}>Fill in the details to promote this user to agency pitcher.</p>
                                <p className="text-xs mt-0.5" style={{ color: "hsl(220 10% 52%)" }}>{user.uid}</p>
                            </div>
                            <form method="dialog">
                        ...
                        <div className="overflow-y-auto px-6 py-6" style={{ maxHeight: "75vh" }}>
                            <form onSubmit={handleAddPitcher} ref={formRef}>
                                ...
                                <div className="flex flex-col items-center gap-2 mb-6">
                                    ...
                                    <input id={photoInputId} name="photo" type="file" accept="image/*" className="hidden" onChange={handlePhotoChange} />
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-3">
                                    <Field id={`pitcher-name-${uniqueId}`} name="name" label="Full Name" type="text" placeholder="e.g. Nafis Hasan" defaultValue={user?.name || ""} icon={<User size={15} />} required />
                                    <Field id={`pitcher-phone-${uniqueId}`} name="phone" label="Phone Number" type="tel" placeholder="+880 1XXX-XXXXXX" defaultValue={user?.phone || ""} icon={<Phone size={15} />} required />
                                    <Field id={`pitcher-email-${uniqueId}`} disabled={true} name="email" label="Email Address" type="email" placeholder="pitcher@example.com" defaultValue={user?.email || ""} icon={<Mail size={15} />} required />

                                    {/* Gender ” DaisyUI select */}
                                    <div className="flex flex-col gap-1.5">
                                        <label htmlFor={`pitcher-gender-${uniqueId}`} className="text-xs font-semibold uppercase tracking-wider" style={{ color: "hsl(220 10% 52%)" }}>Gender</label>
                                        <select id={`pitcher-gender-${uniqueId}`} name="gender" required defaultValue={user?.gender || ""} className="select select-bordered w-full text-sm"
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-3">
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        <label htmlFor={`pitcher-qualification-${uniqueId}`} className="text-xs font-semibold uppercase tracking-wider" style={{ color: "hsl(220 10% 52%)" }}>Max Qualification</label>
                                        <select id={`pitcher-qualification-${uniqueId}`} name="maxQualification" required className="select select-bordered w-full text-sm"
                                            style={{ background: "hsl(222 12% 11%)", borderColor: "hsl(222 10% 18%)", color: "hsl(220 20% 90%)" }}>
                                            <option value="">Select qualification</option>
                                            {QUALIFICATIONS.map((q) => <option key={q} value={q}>{q}</option>)}
                                        </select>
                                    </div>
                                    <Field id={`pitcher-joined-${uniqueId}`} name="joinedAt" label="Joined At" type="date" defaultValue={defaultJoinedAt} icon={<Calendar size={15} />} required />
                                </div>

                                {/*  Professional Details  */}
                                <SectionLabel label="Professional Details" />
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-3">
                                    <Field id={`pitcher-experience-${uniqueId}`} name="experienceYears" label="Experience (years)" type="number" placeholder="e.g. 3" icon={<Briefcase size={15} />} />
                                    <Field id={`pitcher-specialization-${uniqueId}`} name="specialization" label="Specialization" type="text" placeholder="e.g. Startup Pitching" icon={<Briefcase size={15} />} />
                                </div>
                                <div className="mt-4">
                                    <label htmlFor={`pitcher-bio-${uniqueId}`} className="text-xs font-semibold uppercase tracking-wider" style={{ color: "hsl(220 10% 52%)" }}>Short Bio</label>
                                    <div className="relative mt-1.5">
                                        <AlignLeft size={15} className="absolute left-3 top-3 pointer-events-none" style={{ color: "hsl(220 10% 45%)" }} />
                                        <textarea id={`pitcher-bio-${uniqueId}`} name="bio" rows={3} defaultValue={user?.bio || ""} placeholder="A brief description about the pitcher"
                                ...
                                <div className="grid grid-cols-1 gap-4 mt-3">
                                    <div className="flex flex-col gap-1.5">
                                        <label htmlFor={`pitcher-present-address-${uniqueId}`} className="text-xs font-semibold uppercase tracking-wider" style={{ color: "hsl(220 10% 52%)" }}>Present Address</label>
                                        <div className="relative">
                                            <MapPin size={15} className="absolute left-3 top-3 pointer-events-none" style={{ color: "hsl(220 10% 45%)" }} />
                                            <textarea id={`pitcher-present-address-${uniqueId}`} name="presentAddress" rows={2} defaultValue={user?.presentAddress || ""} placeholder="House, Road, Area, City" required
                                                style={{ ...inputStyle, paddingLeft: "2.25rem", resize: "vertical", height: "auto" }} />
                                        </div>
                                    </div>
                                    ...
                                    {!sameAddress && (
                                        <div className="flex flex-col gap-1.5">
                                            <label htmlFor={`pitcher-permanent-address-${uniqueId}`} className="text-xs font-semibold uppercase tracking-wider" style={{ color: "hsl(220 10% 52%)" }}>Permanent Address</label>
                                            <div className="relative">
                                                <MapPin size={15} className="absolute left-3 top-3 pointer-events-none" style={{ color: "hsl(220 10% 45%)" }} />
                                                <textarea id={`pitcher-permanent-address-${uniqueId}`} name="permanentAddress" rows={2} defaultValue={user?.permanentAddress || ""} placeholder="House, Road, Area, City" required
                                                    style={{ ...inputStyle, paddingLeft: "2.25rem", resize: "vertical", height: "auto" }} />
                                            </div>
                                        </div>
                                    )}
                                ...
                                <div className="mt-3">
                                    <label htmlFor={nidInputId} className="flex flex-col items-center justify-center gap-3 px-4 py-5 cursor-pointer transition-colors overflow-hidden" style={{ background: "hsl(222 12% 11%)", border: "1.5px dashed hsl(352 58% 49% / 0.35)", borderRadius: 12, minHeight: 160 }}>
                                        {nidPreview?.length > 0
                                        ...
                                        <input id={nidInputId} name="NIDFile" type="file" accept="image/jpeg,image/png,image/webp" multiple className="hidden" onChange={handleNidChange} />
                                    </label>
                                </div>
                                {nidPreview.length > 0 && (
...
/*  Field component  */
const Field = ({ id, name, label, type, placeholder, icon, required, defaultValue, disabled = false }: {
    id: string; name: string; label: string; type: string;
    placeholder?: string; icon: React.ReactNode; required?: boolean; defaultValue?: string | number; disabled?: boolean
}) => (
    <div className="flex flex-col gap-1.5">
        ...
        <div className="relative">
            ...
            <input id={id} name={name} type={type} placeholder={placeholder} required={required} defaultValue={defaultValue} style={inputStyle} disabled={disabled} />
        </div>
    </div>
);
...
export default ApproveAsPitcher;
